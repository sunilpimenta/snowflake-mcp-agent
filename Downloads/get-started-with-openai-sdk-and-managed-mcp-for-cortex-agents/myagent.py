...
class SnowflakeMCPClient:
    ...
    async def agent_node(self, state: AgentState) -> AgentState:
        ...
        system_msg = SystemMessage(
            content=f"""You are a sales intelligence assistant with access to Snowflake data via MCP tools.
You have access to the following tools: {tools_desc}
When you need data, you can suggest which tool to use. Be helpful and provider insights based on conversation context."""
        )
    ...
    async def test_tools(self):
        ...
        # Test 1: Cortex Search
        print("\n Testing Cortex Search (sales-conversation-search)...")
        try:
            result = await self.call_tool(
                "sales-conversation-search",
                {
                    "query": "budget concerns",
                    "limit": 3
                }
            )
            print(f" Cortex Search results")
            if result.get('content'):
                for item in result['content'][:1]: # show first result only
                    if item.get('type') == 'text':
                        print(f"   Preview: {item['text'][:200]}...")
        except Exception as e:
            print(f" Cortex Search test failed: {str(e)}")
        
        # Test 2: Cortex Analyst
        print("\n Testing Cortex Analyst (sales-metrics-analyst)...")
        try:
            result = await self.call_tool(
                "sales-metrics-analyst",
                {
                    "message": "What is the total deal value by product line?"
                }
            )
            print(f" Analyst query results")
            if result.get('content'):
                for item in result['content'][:1]: # show first result only
                    if item.get('type') == 'text':
                        print(f"   Result: {item['text'][:300]}...")
        except Exception as e:
            print(f" Cortex Analyst test failed: {str(e)}")

        # Test 3: SQL Executor
        print("\n Testing SQL Executor (sales-sql-executor)...")
        try:
            result = await self.call_tool(
                "sales-sql-executor",
                {
                    "query": "SELECT COUNT(*) as total_deals FROM SALES_INTELLIGENCE.DATA.SALES_METRICS;"
                }
            )
            ...
            print(f" SQL Executor test failed: {str(e)}")
